...
RUN . $SPACK_ROOT/share/spack/setup-env.sh && \
      ...
      spack spec octopus@${OCT_VERSION} arpack=on cgal=on debug=off elpa=on libyaml=on likwid=on mpi=off netcdf=on nlopt=on parmetis=on pfft=on python=on && \
      # run the spack installation (adding it to the environment):
      spack add octopus@${OCT_VERSION} arpack=on cgal=on debug=off elpa=on libyaml=on likwid=on mpi=off netcdf=on nlopt=on parmetis=on pfft=on python=on && \
      spack install && \
